...
- [x] 2. モデル定義の一元管理モジュールを作成する
- [x] 2.1 (P) モデル定義の定数と型を実装する
...
- [x] 2.2 (P) モデル定義モジュールのユニットテストを作成する
...
- [x] 4. API ルートの変更（RequestContext 構築とモデルバリデーション）
- [x] 4.1 チャット API に RequestContext 構築とモデルバリデーションを追加する
...
- [x] 4.2 会話作成 API に modelId 保存を追加する
...
- [x] 4.3 会話一覧 API のレスポンスに modelId を含める
  - `GET /api/conversations` の `select` に `modelId: true` を追加し、レスポンスに `modelId` フィールドを含める
  - _Requirements: 3.3_

- [x] 5. フロントエンドのフック変更
- [x] 5.1 useChatSession に modelId パラメータを追加する
...
- [x] 5.2 useConversations に modelId 管理を追加する
...
- [x] 7. ChatContainer にモデル選択状態の管理を統合する
- [x] 7.1 ChatContainer にモデル選択の state 管理を追加する
